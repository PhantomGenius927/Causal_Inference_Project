...
# Outcome Variable
## MID onsets
dyadyrs <- create_dyadyears(directed = FALSE, subset_years = c(1946:2014)) %>%
  add_cow_mids() %>%
  mutate(
...
ggplot(data = da) +
  geom_line(aes(x = year, y = onset), color = "navyblue") + 
  labs(
    title = "Numbers of Dyadic Militarized Interstate Dispute Onsets From 1946 to 2014",
    x = "Year",
    y = "Numbers of Onsets"
  ) + 
...
ggplot(data = da) +
  geom_line(aes(x = year, y = regime)) + 
  labs(
    title = "Numbers of Dyad's with Regime Change From 1946 to 2014",
    x = "Year",
    y = "Numbers of Dyadic Regime Change"
  ) + 
  stevemisc::theme_steve_ms(base_size = 12)
...
idk2 <- feols(treatyr ~ avg_mid, data = demo2)
etable(idk2)

ggplot(demo2, aes(x = treatyr)) +
  geom_smooth(aes(y = avg_mid), method = "lm", se = TRUE) + 
  stevemisc::theme_steve_ms()
...
dead <- aggregate(cowmidonset ~ tau, demo3, mean)
subsetdead <- dead[dead$tau <= 5 & dead$tau >= -5, ]

ggplot(subsetdead, aes(x = tau)) +
  geom_line(aes(y = cowmidonset)) + 
  geom_vline(xintercept = 0)

reg <- feols(cowmidonset ~ factor(tau) | id+year, cluster = c("year", "id"), data = filter(demo3, abs(tau) < 10))
regdat <- as_tibble(reg$coeftable)
ggplot(regdat, aes(x = tau, y = Est)) +
  geom_point()
```
