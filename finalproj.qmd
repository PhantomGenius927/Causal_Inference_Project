...
Hmisc::describe(da, exclude.missing = TRUE, digits = 4, trans = NULL)


# Install and load required packages
library(plm)

# Convert to pdata.frame object

# Descriptive statistics for each variable across time periods
summary_stats_across_time <- pdata.frame(dyadyrs, index = c("id", "year")) %>%
  filter(across(list(mean = mean, sd = sd, median = median, min = min, max = max)))

print("Descriptive Statistics for Each Variable Across Time Periods:")
print(summary_stats_across_time)

# Descriptive statistics for each variable within each time period
summary_stats_within_time <- panel_data_p %>%
  group_by(Time) %>%
  summarise(
    Mean_Age = mean(Age),
    SD_Age = sd(Age),
...
print("Descriptive Statistics for Each Variable Within Each Time Period:")
print(summary_stats_within_time)
...
demo2 <- dyadyrs %>%
  ...
  summarise(
    avg_mid = mean(mid, na.rm = TRUE),
    treatyr_sing = unique(treatyr_sing)
  )

idk2 <- feols(treatyr_sing ~ avg_mid, data = demo2)
etable(idk2)

ggplot(demo2, aes(x = treatyr_sing)) +
  geom_smooth(aes(y = avg_mid), se = TRUE)
