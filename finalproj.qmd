...
# Outcome Variable
## MID onsets
dyadyrs <- create_dyadyears(directed = FALSE, subset_years = c(1945:2014)) %>%
  add_cow_mids() %>%
  mutate(
...
da <- dyadyrs %>%
  ...
  summarise(
    ...
    avg_piva = mean(piva, na.rm = TRUE),
    sd_piva = sd(piva, na.rm = TRUE),
    defense = sum(cow_defense, na.rm = TRUE),
    neutral = sum(cow_neutral, na.rm = TRUE),
    nonagg = sum(cow_nonagg, na.rm = TRUE),
    entente = sum(cow_entente, na.rm = TRUE),
...
ggplot(data = da) +
  ...
  labs(
    title = "Numbers of Dyadic Militarized Interstate Dispute Onsets From 1945 to 2014",
    x = "Year",
    y = "Numbers of Onsets",
...
ggsave(filename = "onset.png", device = "png", 
        dpi = 300, limitsize = FALSE)

ggplot(data = da) +
  geom_line(aes(x = year, y = regime), color = "darkgoldenrod1", linewidth = 0.7) + 
  labs(
    title = "Numbers of Dyad's with Regime Change From 1945 to 2014",
    x = "Year",
    y = "Numbers of Dyadic Regime Change",
    caption = "Source: Polity IV 2018"
  ) + ggthemes::theme_clean(base_size = 10) + 
  theme(text = element_text(family = "Crimson Text"))
ggsave(filename = "regime.png", device = "png", 
        dpi = 300, limitsize = FALSE)
```
...
etable(ees, file = here::here("tables/ees.tex"))
...
ggplot(pta, aes(x = treatyr, y = avg_mid)) +
  ...
  labs(
    ...
    title = "Paralle Trend Analysis",
    caption = "Source: COW MIDs Dataset"
  )
ggsave(filename = "paratrend.png", device = "png", limitsize = FALSE, dpi = 300)
```
...
m2.2 <- feols(cowmidonset ~ regcha_uni + 
                ...
                ccode1 + ccode2 + year, 
              cluster = c("year", "ccode1", "ccode2"), data = dyadyrs)
etable(m2.1,m2.2, file = here::here("tables/2.tex"))
...
m3.4 <- feols(cowmidonset ~ regcha_uni  + 
                ...
                ccode1 + ccode2 + year, 
              cluster = c("year", "ccode1", "ccode2"), data = dyadyrs)
etable(m3.1, m3.2, m3.3, m3.4, file = here::here("tables/etr.tex"))
...
es_reg <- feols(cowmidonset ~ factor(tau) | ccode1 + ccode2 + year, 
                cluster = c("year", "ccode1", "ccode2"), 
                data = filter(es, abs(tau) < 10))

modelsummary::modelsummary(es_reg, stars = TRUE)
...
ggplot(data = es_regdat, aes(x = tau, y = Est)) + 
  ...
  geom_smooth(data = subset(es_regdat, tau>=0), se = TRUE)
ggsave(filename = "leadslags.png", device = "png", limitsize = FALSE, dpi = 300)

es_reg2 <- feols(cowmidonset ~ factor(tau) | id + year, 
                cluster = c("year", "id"), 
                data = filter(es, abs(tau) < 10))

modelsummary::modelsummary(es_reg2, stars = TRUE)

es_regdat2 <- as_tibble(es_reg2$coeftable)
es_regdat2$tau = -8:9
es_regdat2$Est = es_regdat2$Estimate-es_regdat2$Estimate[es_regdat2$tau==0]

ggplot(data = es_regdat2, aes(x = tau, y = Est)) + 
  theme_minimal(base_size = 10) + 
  theme(text = element_text(family = "Crimson Text"))+
  xlab("Years Pre/Post Entry") + 
  ylab("Militarized Interstate Dispute") +
